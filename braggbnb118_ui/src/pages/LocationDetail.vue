					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateLocation()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Locations = true">Add</base-button>
								              <modal :show.sync="modal2Locations">
								  ...
								  <base-input label="Country" type="text" placeholder="Enter Country" v-model="locationToAdd.country"></base-input>
												                  </form>
								                </div>
      ...
      <template>
        ...
        <div class="content">
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
			...
			<td rowspan="50"><img class="location-image" :alt="locationDetails.locationNumber" :src="`/images/${locationDetails.imageFile}`" /></td>
		</tr> 
		
		
										<tr> 
					<td class="detail_view_column_2">Location Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LocationId" type="text" placeholder="Enter LocationId" v-model="locationDetails.locationId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">City</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="City" type="text" placeholder="Enter City" v-model="locationDetails.city"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">State</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="State" type="text" placeholder="Enter State" v-model="locationDetails.state"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Country</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Country" type="text" placeholder="Enter Country" v-model="locationDetails.country"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateLocation() {
      ...
      const jsonData = JSON.stringify(this.locationDetails);
      const res = await LocationService.update(jsonData);
...
//        this.getAllLocations();
    ...
    async getLocationDetails() {
      try {
			let response = await LocationService.get(this.locationId);
